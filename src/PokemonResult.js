...
export default class PokemonResult extends React.Component {
	...
	render() {
		...
		return (
			...
			<div className="mainResult">
				...
				<EvolutionChain evolutions={this.state.evolutions} capitalizeItems={this.capitalizeItems} />
			</div>
		)
	}

	componentDidMount() {
		let searchedPokemon = this.props.match.params.name;
	...
	componentWillReceiveProps(nextProps) {
		let searchedPokemon = nextProps.match.params.name;
		if  (searchedPokemon === 'smudge') {
			return this.setState({
				pokemon: 'Smudge',
				height: 0,
